...
class ModbusRTUTest {
    ...
    {
        val modbusSensor = ModbusRTU( "COM1" )
        ...
        assertEquals( "COM1", modbusSensor.settings.description )
    }

    @Test fun testCreateModbusRTUParams()
    {
        val modbusSensor = ModbusRTU( "COM1",5, 1200 )
    ...
    @Test fun testSetMultipleHoldingValues() {
        val modbusSensor = ModbusRTU( "COM1" )
        val messageToSend: Array<Byte> = arrayOf(0x01, 0x10, 0x00, 0x45, 0x00, 0x05, 0x0A, 0x00, 0x01, 0x00,
                0x02, 0x00, 0x03, 0x00, 0x04, 0x00, 0x05, 0xAE.toByte(), 0xEF.toByte())
        val messageToRead: Array<Byte> = arrayOf(0x01, 0x10, 0x00, 0x45, 0x00, 0x05, 0x11, 0xDF.toByte())

        modbusSensor.port = PortCOMMock( messageToSend, messageToRead)
        val success = modbusSensor.SetMultipleHoldingValues( 70, arrayOf(1,2,3,4,5))
    ...
    @Test fun testSetHoldingSwFloatReg60Val1337() {
        val modbusSensor = ModbusRTU( "COM1" )
        val messageToSend: Array<Byte> = arrayOf(0x01, 0x10, 0x00, 0x3B, 0x00, 0x02, 0x04, 0x44, 0xA7.toByte(),
                0x20, 0x00, 0x0D, 0xDB.toByte())
        val messageToRead: Array<Byte> = arrayOf(0x01, 0x10, 0x00, 0x3B, 0x00, 0x02, 0x30, 0x05)

        modbusSensor.port = PortCOMMock( messageToSend, messageToRead)
        val success = modbusSensor.SetHoldingSwFloat(60, 1337f)
